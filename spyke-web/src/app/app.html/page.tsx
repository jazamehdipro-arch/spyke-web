...
function ContratsV1({
  ...
  async function sendContractForSignature(contractId: string) {
    try {
      ...
      // Open the email composer with the signing link + the exact same PDF that will be displayed in the signing link.
      // We fetch the PDF generated by /api/contract-sign/create-link (stored in signed_contracts bucket) to avoid mismatches.
      const signToken = String((linkJson as any)?.token || '').trim()
      ...
      await openMailComposeWithAttachment({
        ...
        text,
        getBlob: async () => {
          if (!signToken) throw new Error('Token signature indisponible')
          const infoRes = await fetch(`/api/contract-sign/get?token=${encodeURIComponent(signToken)}`)
          const infoJson = await infoRes.json().catch(() => ({}))
          if (!infoRes.ok) throw new Error(String((infoJson as any)?.error || 'PDF indisponible'))
          const pdfUrl = String((infoJson as any)?.pdfUrl || '')
          if (!pdfUrl) throw new Error('PDF indisponible')
          const pdfRes = await fetch(pdfUrl)
          const blob = await pdfRes.blob()
          if (!pdfRes.ok) throw new Error((await blob.text()) || 'Erreur PDF')
          return { blob, token }
        },
