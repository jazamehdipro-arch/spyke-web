...
export async function POST(req: Request) {
  try {
    ...
    const nameKey = find(['nom', 'name', 'prénom', 'prenom', 'contact', 'pseudo'])
    const platformKey = find(['plateforme', 'platform', 'réseau', 'reseau', 'canal', 'channel'])
    const followersKey = find(['abonnés', 'abonnes', 'followers', 'audience', 'communauté'])
    ...
    const contacts = rows
      .map(row => ({
        ...
        platform: platformKey ? String(row[platformKey] ?? '').trim() || null : null,
        followers: followersKey ? String(row[followersKey] ?? '').trim() || null : null,
      }))
