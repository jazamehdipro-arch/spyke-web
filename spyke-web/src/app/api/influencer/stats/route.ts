...
export async function GET(req: Request) {
  try {
    ...
    const [{ count: total }, { count: pending }, { count: sent }, { count: error }] = await Promise.all([
      supabaseAdmin.from('influencer_contacts').select('id', { count: 'exact', head: true }),
      supabaseAdmin.from('influencer_contacts').select('id', { count: 'exact', head: true }).eq('status', 'pending'),
      supabaseAdmin.from('influencer_contacts').select('id', { count: 'exact', head: true }).eq('status', 'sent'),
      supabaseAdmin.from('influencer_contacts').select('id', { count: 'exact', head: true }).eq('status', 'error'),
    ])

    const { data: recent } = await supabaseAdmin
      .from('influencer_contacts')
      .select('id, email, name, platform, followers, status, sent_at, created_at')
